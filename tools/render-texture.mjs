#!/usr/bin/env node
// Realistic pizza texture renderer (PIL/Pillow).
// - Crust with golden gradient + irregular edge
// - Tomato sauce with subtle variation
// - Melted cheese layer with translucent spots
// - Per-ingredient icons in a circular layout
// - Recipe title + subtitle text with shadow
// - Optional: highlight herbs (green specs) for ingredients tagged as herbs
//
...
// Per-ingredient realistic colors (RGB)
const ING_COLORS = {
  'Mozzarella': [255, 248, 230],
  'Tomato': [220, 50, 47],
  'PizzaSauce': [200, 40, 40],
  'Basil': [50, 140, 60],
  'Bacon': [150, 70, 50],
  'Chicken': [200, 170, 110],
  'CookedChicken': [200, 170, 110],
  'Jalapeño': [80, 160, 70],
  'Jalapeno': [80, 160, 70],
  'Pepperoni': [180, 50, 40],
  'Salami': [170, 60, 50],
  'Mushroom': [220, 200, 180],
  'Olive': [60, 70, 50],
  'Pineapple': [240, 220, 80],
  'Ham': [220, 150, 150],
  'Onion': [240, 230, 220],
  'Pepper': [80, 200, 80],
  'Sausage': [180, 120, 90],
  'Ricotta': [255, 252, 240],
  'Parmesan': [240, 230, 180],
  'Gorgonzola': [200, 210, 230],
  'Anchovy': [120, 100, 70],
  'Artichoke': [140, 170, 90],
  'Arugula': [80, 130, 70],
  'Pesto': [60, 120, 50],
  'Truffle': [80, 60, 40],
  'Garlic': [250, 245, 230]
};
const HERBS = ['Basil', 'Arugula', 'Pesto', 'Oregano', 'Parsley'];
const hash = (s) => { let h = 0; for (let i = 0; i < s.length; i++) h = ((h << 5) - h + s.charCodeAt(i)) | 0; return Math.abs(h); };
const colorOf = (s) => ING_COLORS[s] || [200, 100 + (hash(s) % 100), 100 + ((hash(s) >> 4) % 80)];
...
ing_color_map = ${JSON.stringify(ING_COLORS)}
...
for ing in ingredients:
    if ing['IngredientID'] in ${JSON.stringify(HERBS)}:
        for _ in range(8):
            a = random.uniform(0, 2 * math.pi)
...
try {
  ...
  console.error('Texture render failed:', e.message);
  process.exit(1);
}
